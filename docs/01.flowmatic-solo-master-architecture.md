# FlowMatic-SOLO R2: Complete Master Architecture
...
- **Audio Support**: Voice announcements
...
POST   /api/terminal/transfer           # Transfer to another service
...
#### **Layout**
```
Header: Company Logo + Current Time
...
Footer: Help button + Language toggle
```

#### **User Flow**
1. Select service → See current queue info
2. Choose delivery method (print/SMS/email)
3. Enter details if needed (phone/email)
4. Confirm → Generate ticket
5. Show ticket details + estimated wait time
...
#### **Layout**
```
Header: Agent name + Counter info + Time
...
4. Handle special cases (recall, park, transfer, etc.)
